...
public class FindNearWorkByParamPager extends AbstractExecuter {
    ...
    public Object execute(Object[] params) {
        ...
        executeSql.append("ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN((? * PI() / 180 - work_latitude * PI() / 180) / 2),2) + COS(? * PI() / 180) * COS(work_latitude * PI() / 180) * POW(SIN((? * PI() / 180 - work_longitude * PI() / 180) / 2),2))) * 1000) AS distance ");
